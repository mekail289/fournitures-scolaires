...
const verifiedPrices:Price[]=[
 ...
 ...["4-01","6-01"].map(itemId=>({itemId,merchantId:"commande-scolaire" as const,packagePrice:2.99,packageQuantity:1,available:true,productName:"Cahier ligné broché, 3 trous, 0,7 cm, 27,6 × 21,3 cm, 32 pages",sourceUrl:"https://commandescolaire.com/liste/ecole-primaire-de-la-seigneurie-prescolaire-2-3/",verifiedAt:checkedAt})),
...
export const initialState:AppState={items:seedItems,inventory:[],prices:verifiedPrices,selectedMerchants:merchants.map(x=>x.id),coupon:{active:true,value:10},promotions:merchants.map(m=>({merchantId:m.id,code:"",active:false,kind:"fixed",value:0})),merchantSettings:merchants.map(m=>({merchantId:m.id,mode:"store",shippingFee:0})),plan:[],history:[]};
