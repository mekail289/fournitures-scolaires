...
export type Price={itemId:string;merchantId:MerchantId;packagePrice:number;packageQuantity:number;available:boolean;productName?:string;sourceUrl?:string;verifiedAt?:string;promotionLabel?:string;regularPrice?:number;matchStatus?:"verified"|"candidate"|"rejected";matchNote?:string};
...
export type AppState={items:Supply[];inventory:Inventory[];prices:Price[];selectedMerchants:MerchantId[];postalCode?:string;coupon:{active:boolean;value:number;expires?:string};promotions?:Promotion[];merchantSettings?:MerchantSetting[];plan:PlanLine[];history:{date:string;label:string;amount:number}[]};
